...
//		var educPortion = dc.portionTextChart("#educ-text-graph", "main");
		...
		function percentageFactor(group,factor) {
			recordsTotal = group.size()
			for (i=0;i < group.all(); i++) {
				console.log(x)
			}
		}
			...
			var isin = group.reduceCount().all().filter(function(d) {return d.key == factor})
		console.log(isin[0].value/recordsTotal);	
			if (isin[0]) {
				console.log("hello")
				return isin[0].value/recordsTotal;
				} else { 
					return 0; 	
			}
...
//		console.log(groupLang.all())
		console.log(percentageFactor(groupLang, "Russian"))
	
//		console.log(groupLang.all().filter(function(d) {return d.key == "Russian"})[0].value)
//		console.log(groupLang.all().filter(function(d) {console.log(d)}))
		var langPortion = dc.numberDisplay("#lang-text-graph", "main");
//		var educPortion = dc.numberDisplay("#educ-text-graph", "main");
		internetTreeChart
			.dimension(intDim)
			.removeThese(["undefined","Difficult to answer"]);

		langPortion
			.valueAccessor(function(d) { return d.reduceCount().all().filter(function(d) {return d.key == "Russian"})[0].value;} )
			.group(groupLang);

//		educPortion
//			.dimension(educDim)
//			._codeBook({"Primary school (finished the primary school, a 4-9 year pupi":"Primary","Secondary school (finished 9 years, a 10-11 year pupil)":"Some Secondary","Complete secondary school (finished 10-11 years)":"Secondary","Vocational school, training\\further training centre for work":"Vocational School","Technical secondary school, college, other specialised secon":"Technical","Higher educational institution (Specialist or Master degree)":"Masters","DK":"DK"})
